...
{
    for (int i = 0; i < m_StackSize; i++) m_Delays[i] = Delays[i];
}

void StackedBoxCarDriver::SetWidths(double *Widths)
{
    for (int i = 0; i < m_StackSize; i++) m_Widths[i] = Widths[i];
}
...
double StackedBoxCarDriver::GetValue(double Time)
{
    ...
    for (int i = 0; i < m_StackSize; i++)
    {
        ...
        NormalisedCycleTime = fmod(Time, m_CycleTimes[i]) / m_CycleTimes[i];
