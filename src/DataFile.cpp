...
// returns true on error
bool DataFile::ReadNextLine2(char *line, int size, bool ignoreEmpty,
                            const char *commentString, const char *continuationString)
{
    ...
    while (loopFlag)
    {
        ...
        if (continuationString)
        {
            ...
            {
                loopFlag = true;
                int len = int(strlen(c) - strlen(continuationString)); // this may not be 64 bit safe
                line = line + len;
                size = size - len;
            }
        }
...
// Count token utility
int DataFile::CountTokens(const char *string)
{
    const char *p = string;
    bool inToken = false;
    int count = 0;
...
// Count token utility
int DataFile::CountTokens(const char *string, const char *separators)
{
    const char *p = string;
    bool inToken = false;
    int count = 0;
...
// (this is probably an error status)
// recommend that tokens are counted first
int DataFile::ReturnTokens(char *string, char *ptrs[], int size, const char *separators)
{
    char *p = string;
    bool inToken = false;
    int count = 0;
...
bool DataFile::StringEndsWith(const char * str, const char * suffix)
{
    ...
    if (strncmp(str + str_len - suffix_len, suffix, suffix_len) == 0) return 1;
    return 0;
}
...
void DataFile::Double(const char *buf, int n, double *d)
{
    char *ptr;
    d[0] = strtod(buf, &ptr);
    for (int i = 1; i < n; i++)
...
void DataFile::Int(const char *buf, int n, int *d)
{
    char *ptr;
